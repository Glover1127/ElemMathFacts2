...
def advance_level():
    ...
    if st.session_state.current_level == 14:
        ...
        st.success(f"Great work! You've completed Level {st.session_state.current_level}. Moving on to Level {st.session_state.current_level + 1}.")
...
if st.session_state.current_level is None:
    ...
    if st.button("Start Game"):
        ...
        st.experimental_rerun()
    st.stop()  # Prevent the rest of the app from running until a level is selected
...
if submitted:
    # Validate that the answer is an integer
    try:
        if int(answer) == a + b:
